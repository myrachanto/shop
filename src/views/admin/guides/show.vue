...
<v-container class="">
          ...
          >
           ...
           >
            ...
            <v-row>
              <v-col md="6">
            <v-card-title class="display-1">guide Name:   {{form.name}}</v-card-title>
            <v-card-title >Title:   {{form.title}}</v-card-title>
            <v-card-title >Meta tag:   {{form.meta}}</v-card-title>            
            <h2 >Specs</h2>
            <div v-if="form.prods">
              <ul v-for="(item,i) in form.prods" :key="i">
                  <li class="greens">{{item.name}}</li>
              </ul>
            </div>
              </v-col>
              <v-col md="6">
            ...
            <v-row>
              <v-col md="12">
                Summary:  <p v-html="form.summary"></p>
              </v-col>
            </v-row>
            </v-container>
